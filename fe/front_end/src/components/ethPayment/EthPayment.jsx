...
const EthPayment = ({ orderId, amountVND, onSuccess }) => {
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            } else if (err.code === -32002 || (err.info && err.info.error && err.info.error.code === -32002)) {
                setError('Yêu cầu kết nối ví đang chờ. Vui lòng mở MetaMask để xác nhận!');
            } else {
    ...
    return (
        <div className="mt-4 p-4 border rounded bg-gray-50">
            ...
            <button
                ...
                disabled={loading}
                className={`w-full py-2 px-4 rounded text-white font-bold ${loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-orange-500 hover:bg-orange-600'
                    }`}
            >
